...
- **DECISION:** Because this stack is Python-only (FastAPI backend, no Swift/ObjC layer), a future Network Extension–based `TrafficProvider` is not a natural upgrade path the way it would have been for a native app — `NetworkExtension.framework` entitlements are granted to signed native apps, not to a Python process. If deeper packet-level capture is ever needed beyond what mitmproxy's local mode offers, that requires either a separate small native helper (its own project) or accepting the mitmproxy-based approach as this project's ceiling. See `DECISIONS.md`.
