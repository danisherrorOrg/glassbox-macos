...
### ADR-016: `AF_UNIX` (local IPC / Unix domain sockets) is out of scope for `SocketProvider`
**Status:** Accepted. Recorded 2026-09-07 in response to a Phase 0.1/0.2 code-review finding: `SocketProvider`'s `netstat2` call (`src-tauri/src/providers/socket.rs`) has only ever requested `AddressFamilyFlags::IPV4 | AddressFamilyFlags::IPV6` — `AF_UNIX` sockets were excluded from the first line of code that queried sockets at all, with no ADR or comment saying so. The exclusion itself was correct; what was missing was the record that it's a decision, not an oversight nobody got around to.\
**Decision:** `SocketProvider` observes IPv4/IPv6 TCP and UDP sockets only. `AF_UNIX` sockets (local IPC — the mechanism behind, for instance, a browser talking to its own helper processes, or many macOS system services) are not enumerated, tracked, or exposed through `NetworkConnection`, now or as a planned near-term addition.\
**Why:** this project is a *network* inspector — the report's framing (`process-network-inspector-report.md`'s opening: "a process can speak TCP, UDP, QUIC, HTTP, HTTPS, WebSockets, gRPC, Unix domain sockets, or custom binary protocols") names Unix domain sockets only as one of the protocols a process *can* speak, in service of framing how much visibility varies — it was never a promise to observe all of them. `AF_UNIX` sockets don't cross the machine's network boundary; nothing about "what is this process talking to on the network" requires seeing them, and every downstream concept this project is built around — `remote_addr`/`remote_port`, DNS/hostname correlation (Phase 0.2), HTTP/HTTPS observation (Phase 0.3+) — presumes an actual network peer, which `AF_UNIX` sockets don't have.\
**What this doesn't rule out:** if a future need genuinely requires local-IPC visibility (e.g. correlating a process's Unix-socket peers as part of a richer process picture), that's a new, explicitly-scoped feature to design from scratch — including its own connection-identity story, since `docs/DATA_MODEL.md`'s matching rule (`pid, protocol, local_addr, local_port, remote_addr, remote_port`) assumes IP-addressable endpoints throughout. Not something this ADR should be read as quietly leaving room for by omission.
