...
`TrafficProvider`'s interface is the only thing that needs reimplementing if the capture strategy ever changes. Because this is now a Python-only stack, a Network Extension–based implementation isn't achievable as a pure Python component — that entitlement is granted to signed native apps. It would require introducing a separate, separately-signed native helper alongside this stack, not a Python-only upgrade. See `DECISIONS.md` for the full trade-off.
