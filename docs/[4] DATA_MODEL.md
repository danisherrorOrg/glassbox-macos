...
**The actual matching algorithm, per the Phase 0.3 spike (`docs/[9] TODO.md`, `docs/PERMISSIONS_AND_PLATFORM.md`):** `(pid, remote_addr, remote_port)` only — `local_addr`/`local_port` are excluded, not merely optional, because the spike found `mitmproxy`'s `local:<pid>` capture never exposes a usable value for them (`client_conn` reflects the local redirector's own loopback stub connection, not the process's real originating socket). `pid` itself comes from the Rust core's own knowledge of which pid a capture session targets, not from the evidence a provider produces (a `TrafficProvider` has no reliable way to read it back off a flow either — confirmed empirically, not merely assumed unavailable).

**Match candidates are not restricted to `NetworkConnection`s in `discovered`/`active` — unlike the socket-to-socket matching rule above.** This was tried first, by analogy with that rule, and found wrong by the Phase 0.3 spike's own integration test: real HTTP/1.0 requests routinely complete and their connection expires *faster* than the socket-polling interval, so by the time a captured flow is available to correlate, its connection has often already left `active`. A `connection_id` stays valid and meaningful for the life of the tracked object regardless of its current `lifecycle_state`, so `expired`/`closed` connections remain valid correlation targets — the zero-or-multiple-candidates ambiguity check (still in force, per the split-over-merge policy) is what actually prevents a wrong guess, independent of lifecycle state.
